"""ETF 글로벌 트렌드 포트폴리오 대시보드 — Streamlit 페이지."""
...
HISTORY_PATH = Path(__file__).parent.parent / "data" / "etf_portfolio_history.json"
RISK_COLOR = {"낮음": "#34C759", "중간": "#FF9500", "높음": "#FF3B30"}
ACTION_COLOR = {
...
with col_yt:
    ...
    yt_queries = topic.get("youtube_search_queries", [])
    yt_items = "".join(
...
with col_pie:
    if portfolio:
        tickers_p = [e["ticker"] for e in portfolio]
        weights_p  = [e["weight"]  for e in portfolio]
        colors_seq = [
            "#007AFF", "#FF9500", "#34C759", "#FF3B30", "#AF52DE",
            "#5AC8FA", "#FF2D55", "#FFD60A",
        ]
        fig_pie = go.Figure(
            go.Pie(
                labels=tickers_p,
                ...
                hole=0.45,
                marker=dict(
                    colors=colors_seq[: len(tickers_p)],
    ...
    if portfolio:
        for e in portfolio:
            ...
            st.markdown(
                ...
                f'<span class="badge badge-blue">{cat}</span>'
...
st.markdown(
    ...
    f'</div>'
    f'<div style="color:#ccc;font-size:0.9rem;line-height:1.7;margin-bottom:14px">{overall_comment}</div>',
    unsafe_allow_html=True,
)
...
if changes:
    for ch in changes:
        ...
        delta_str = (
            f'+{delta:.1f}%p' if delta > 0
            else f'{delta:.1f}%p' if delta < 0
            else "—"
        )
        st.markdown(
...
# ── Section 5: ETF 현재 시세 & Performance ────────────────────────────────────
...
if portfolio:
    ...
    if perf_df.empty:
        ...
        table_html = f"""
        <div style="background:#1a1d2e;border-radius:12px;border:1px solid #2a2d40;overflow:hidden">
        ...
        tab_labels = list(periods_map.keys())
        tabs = st.tabs(tab_labels)

        for tab, (label, col_key) in zip(tabs, periods_map.items()):
            with tab:
                chart_data = perf_df[["Ticker", col_key]].dropna()
                if chart_data.empty:
                    st.info("데이터 없음")
                    continue
                chart_data = chart_data.sort_values(col_key, ascending=True)
                bar_colors = [PERF_GREEN if v >= 0 else PERF_RED for v in chart_data[col_key]]

                fig_bar = go.Figure(
                    go.Bar(
                        x=chart_data[col_key],
                        y=chart_data["Ticker"],
                        orientation="h",
...
with col_risk_list:
    st.markdown(
        '<div class="card">'
        '<div style="font-weight:700;margin-bottom:10px;color:#FF3B30">⚠ 주요 리스크</div>',
        unsafe_allow_html=True,
    )
    for risk_item in market_outlook.get("key_risks", []):
        st.markdown(
            f'<div style="display:flex;gap:8px;margin-bottom:6px">'
            f'<span style="color:#FF3B30">•</span>'
            f'<span style="color:#ddd;font-size:0.88rem">{risk_item}</span></div>',
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)
...
with col_opp_list:
    st.markdown(
        '<div class="card">'
        '<div style="font-weight:700;margin-bottom:10px;color:#34C759">✦ 투자 기회</div>',
        unsafe_allow_html=True,
    )
    for opp_item in market_outlook.get("opportunities", []):
        st.markdown(
            f'<div style="display:flex;gap:8px;margin-bottom:6px">'
            f'<span style="color:#34C759">•</span>'
            f'<span style="color:#ddd;font-size:0.88rem">{opp_item}</span></div>',
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)
